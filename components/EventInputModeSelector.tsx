...
interface EventInputModeSelectorProps {
  onSelectMode: (mode: 'manual' | 'upload') => void;
  onCancel: () => void;
}

export function EventInputModeSelector({ onSelectMode, onCancel }: EventInputModeSelectorProps): React.ReactElement {
  const options = [
    { mode: 'manual' as const, icon: PencilSquareIcon, title: 'Isi Formulir Manual', description: 'Lengkapi kolom informasi acara satu per satu.' },
    { mode: 'upload' as const, icon: ArrowUpTrayIcon, title: 'Unggah TOR/Dokumen', description: 'Biarkan AI memproses dari file TOR (.pdf, .docx, .txt).' },
  ];
