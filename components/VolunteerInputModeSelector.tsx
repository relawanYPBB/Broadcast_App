...
interface VolunteerInputModeSelectorProps {
  onSelectMode: (mode: 'manual' | 'upload') => void;
  onCancel: () => void;
}

export function VolunteerInputModeSelector({ onSelectMode, onCancel }: VolunteerInputModeSelectorProps): React.ReactElement {
  const options = [
    { mode: 'manual' as const, icon: PencilSquareIcon, title: 'Isi Formulir Manual', description: 'Lengkapi kolom informasi satu per satu.' },
    { mode: 'upload' as const, icon: ArrowUpTrayIcon, title: 'Unggah Dokumen', description: 'Biarkan AI memproses dari file (.pdf, .docx, .txt).' },
  ];
